...
function TodoModifyComponent({id, setResult, moveToList, moveToBack}) {
    ...
    return (
        ...
        <Box sx={{p:1}}>
            <TextField variant='outlined' fullWidth name={'title'} label={'title'} value={String(todo.title)}
            onChange={(e) => changeTodo(e)}></TextField>
        </Box>
        <Box sx={{p:1}}>
            <TextField variant='outlined' fullWidth name={'writer'} label={'writer'} value={String(todo.writer)} onChange={(e) => changeTodo(e)}></TextField>
        </Box>
        <Box sx={{p:1}}>
            <FormControlLabel control={<Checkbox checked={Boolean(todo.complete)}/>} onChange={(e) => changeTodoComplete(e)} label='Complete'/>
        </Box>

...
function TodoReadComponent({id, moveToList, setResult, moveToModify}) {
    ...
    return (
        <>
            ...
            <Box sx={{p:1}}>
                <TextField variant='outlined' fullWidth name={'writer'} label={'writer'} aria-readonly={true} value={String(todo.title)}></TextField>
            </Box>
            <Box sx={{p:1}}>
                <TextField variant='outlined' fullWidth name={'writer'} label={'writer'} aria-readonly={true} value={String(todo.writer)}></TextField>
            </Box>
            <Box sx={{p:1}}>
                <FormControlLabel control={<Checkbox checked={Boolean(todo.complete)}/>} label='Complete'/>
            </Box>

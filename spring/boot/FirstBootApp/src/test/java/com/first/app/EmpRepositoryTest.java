...
//                        .deptno(30)
    ...
    public void empJoinDeptTest(){
        ...
        Page<Emp> page = empRepository.findEmpWithDept(PageRequest.of(0, 5));
    ...
    public void deptWithCntTest(){
        ...
        for(Object[] arr : result.getContent()){
            log.info(Arrays.toString(arr)); // [Dept(deptno=10, dname=ACCOUNTING, loc=NEW YORK), 3], [Dept(deptno=20, dname=RESEARCH, loc=DALLAS), 5], [Dept(deptno=30, dname=SALES, loc=CHICAGO), 7]
        }
    }
    ...
    public void dtoListTest(){
        ...
        for(ListItemDTO itemDTO : list){
            log.info(itemDTO);  // ListItemDTO(empno=7369, ename=SMITH, job=CLERK, dname=RESEARCH, loc=DALLAS) 이런 식
        }
    }
